...
{
    public class UserGroupActionFilterAttribute : FilterAttribute, IActionFilter
    {
        ...
        private const string HEADER_GROUPID = "groupSid";
        ...
        private void InternalActionExecuting(HttpActionContext actionContext, CancellationToken cancellationToken)
        {
            ...
            if (actionContext.Request.Headers.Contains(HEADER_GROUPID))
            {
                groupId = new Guid(actionContext.Request.Headers.GetValues(HEADER_GROUPID).First());
            }
            else
            {
                HttpResponseMessage response = new HttpResponseMessage(HttpStatusCode.PreconditionFailed);
                response.RequestMessage = actionContext.Request;
                response.ReasonPhrase = "Header " + HEADER_GROUPID + " não encontrado.";
                actionContext.Response = response;
            }
        }       
    }
