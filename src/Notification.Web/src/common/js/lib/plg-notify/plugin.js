...
var plgnotify = function ( sysconfig ) {
	...
	function showMessage( dom ) {
		var modal, header, body, footer, text, title, fecharX, btnFechar;
		...
		modal = '<div class="plgmodal" id="plgmodal" aria-hidden="true">' +
				...
				'<a href="#" class="plgmodal-btn-close" aria-hidden="true">×</a>' +
				'</div>' +
				'<div class="plgmodal-body">' +
				'<p> <span class="plgloader"></span> </p>' +
				'</div>' +
				'<div class="plgmodal-footer">' +
				'<a href="#" class="plgmodal-btn">Fechar</a>' +
				'</div></div></div>';
		...
		fecharX.href   = lastHref;
		btnFechar.href = lastHref;
	...
	function openModal( e ) {
		...
		//salva referência da url atual
		lastHref = window.location.href;
		//se não tiver hash, adiciona para evitar reload.
		lastHref = lastHref.match( '#' ) ? lastHref : lastHref + '#';

		//Exibe mensagem
		showMessage( e.target );

		//Redireciona para modal ( verificar se pode dar conflito com rotas angular )
		window.location.href = '#plgmodal';
	...
	function createLayout() {
		...
		if ( !layout.domplugin ) {
			...
			css = '.plgsnackbar{visibility:hidden;width:250px;margin-left:-125px;background-color:#333;color:#fff;text-align:center;border-radius:2px;padding:16px;position:fixed;z-index:150;left:50%;bottom:0;opacity:0}' +
				  '.plgsnackbar span{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}' +
				  '.plgsnackbar-header{font-weight:700}' +
				  '.plgloader{border:8px solid #f3f3f3;border-top:8px solid #3498db;border-radius:50%;width:36px;height:36px;animation:2s linear infinite spin}' +
				  ...
				  '.draggable,.draggable>*,.notificacoes li{display:inline-block}' +
				  '.draggable>i{cursor:move}' +
				  '.hide{display:none!important}' +
				  '.hitbox{pointer-events:none;padding:0;margin:0}' +
				  ...
				  '.notificacoes li{width:100%;padding:8px 0 8px 8px;border-bottom:1px solid rgba(235,238,240,.31);font-size:1rem;overflow:hidden;height:42px}' +
				  '.notificacoes p,.notificacoes span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden;max-width:90%;min-width:30%}' +
				  '.notificacoes li.lida{opacity:.5}' +
				  '.notificacoes li:last-child{border-bottom:0}' +
				  '.notificacoes li:nth-child(odd){background:#fafbfb}' +
				  '.notificacoes li.urgente{background:#d11d1d;color:#fff}' +
				  '.notificacoes li.urgente *{color:#fff}' +
				  '.notificacoes li span{font-size:1.2rem;display:block;font-weight:900;color:#768e99}' +
				  '.circulo,.float-menu{border-radius:100%;width:80px;height:80px}' +
				  '.circulo{box-sizing:border-box;overflow:hidden}' +
				  '.float-menu{display:block;position:fixed;user-select:none;color:#fff;box-shadow:4px 4px 4px rgba(0,0,0,.3);font-family:sans-serif}' +
				  '.float-menu .lateral{width:38px;left:42px;position:absolute;height:40px}' +
				  '.float-menu .lateral a{display:inline-block;width:100%;text-align:center;padding:7px;box-sizing:border-box;background:#f32f2f;font-weight:600;font-size:15px;cursor:pointer;border-radius:100%;height:40px}' +
				  '.float-menu a.numeracao{background:#ff9800;cursor:default;position:absolute;right:0;top:-10px;height:16px;width:16px;padding:5px;margin:auto;border-radius:100%;font-weight:700;z-index:5}' +
				  '.float-menu a.numeracao:hover{background:#ffd200;color:#000}' +
				  '.float-menu .lateral a.esconder svg{margin-top:0}' +
				  '.float-menu .lateral a:hover{background:#a91b1b}' +
				  '.float-menu .lateral a svg{fill:#fff;margin-top:7px}' +
				  '.float-menu .lateral a:first-child{border-bottom:0;border-radius:0 100% 0 0}' +
				  '.float-menu .lateral a:last-child{border-bottom:0;border-radius:0 0 100%}' +
				  '.sino{width:50px;height:80px;cursor:pointer;padding:20px 0 0 10px;box-sizing:border-box;position:relative;background-color:#232b38}' +
				  '.sino svg{fill:#fff}' +
				  '.sino:hover{background:#3d4d60}' +
				  '.shake-anime{animation:1s cubic-bezier(.36,.07,.19,.97) both shake;transform:translate3d(0,0,0);backface-visibility:hidden;perspective:1000px}@keyframes shake{10%,90%{transform:translate3d(-1px,0,0)}20%,80%{transform:translate3d(2px,0,0)}30%,50%,70%{transform:translate3d(-4px,0,0)}40%,60%{transform:translate3d(4px,0,0)}}' +
				  '.plgmodal-btn{background:#428bca;border:1px solid #357ebd;border-radius:3px;color:#fff;display:inline-block;font-size:14px;padding:8px 15px;text-decoration:none;text-align:center;min-width:60px;position:relative;transition:color .1s ease}' +
				  '.plgmodal-btn:hover{background:#357ebd}' +
				  '.plgmodal-btn.plgmodal-btn-big{font-size:18px;padding:15px 20px;min-width:100px}' +
				  '.plgmodal-btn-close{color:#aaa;font-size:30px;text-decoration:none;position:absolute;right:5px;top:0}' +
				  '.plgmodal-btn-close:hover{color:#000}' +
				  '.plgmodal:before{content:"";display:none;background:rgba(0,0,0,.6);position:fixed;top:0;left:0;right:0;bottom:0;z-index:210}' +
				  '.plgmodal:target:before{display:block}' +
				  '.plgmodal:target .plgmodal-dialog{-webkit-transform:translate(0,0);-ms-transform:translate(0,0);transform:translate(0,0);top:20%}' +
				  '.plgmodal-dialog{background:#fefefe;border:1px solid #333;border-radius:5px;margin-left:-200px;position:fixed;left:50%;top:-100%;z-index:211;width:360px;-webkit-transform:translate(0,-500%);-ms-transform:translate(0,-500%);transform:translate(0,-500%);-webkit-transition:-webkit-transform .2s ease-out;-moz-transition:-moz-transform .2s ease-out;-o-transition:-o-transform .2s ease-out;transition:transform .2s ease-out}' +
				  '.plgmodal-body{padding:20px}' +
				  '.plgmodal-footer,.plgmodal-header{padding:10px 20px}' +
				  '.plgmodal-header{border-bottom:1px solid #eee}' +
				  '.plgmodal-header h2{font-size:20px}' +
				  '.plgmodal-footer{border-top:1px solid #eee;text-align:right}li.plgnot>*{pointer-events:none}li.plgnot:hover{background-color:rgba(112,128,144,.42)!important}li.plgnot:hover>*{color:#000!important}';
